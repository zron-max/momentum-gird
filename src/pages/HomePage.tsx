...
const HabitTrackingSVG = () => (
  ...
  >
    ...
    <polyline
      points="60,150 110,130 160,140 210,120 260,125 310,110 360,115"
      fill="none"
      stroke="#6366f1"
      strokeWidth="4"
      strokeLinecap="round"
...
const LearningGoalSVG = () => (
  ...
  >
    ...
    />
    <path
      d="M80 140 C 100 150, 280 150, 300 140 L 320 50 L 60 50 Z"
      fill="#fff"
      opacity="0.2"
      transform="rotate(-5 190 95)"
    />
    <rect
      x="100"
      y="60"
...
const TimeBlockingSVG = () => (
  ...
  >
    ...
    <rect
      ...
      opacity="0.1"
    />
    <text x="50" y="65" fill="white" fontSize="14" fontWeight="bold">
    ...
    <rect
      ...
      opacity="0.1"
    />
    <text x="50" y="118" fill="white" fontSize="14" fontWeight="bold">
      12:00 Lunch
    </text>
    <rect
      x="210"
      ...
      opacity="0.1"
    />
    <text x="220" y="118" fill="white" fontSize="14" fontWeight="bold">
      14:00 Meeting
    </text>
    <defs>
      <linearGradient
...
const HomePage = () => {
    ...
    try {
      ...
      if (
        ...
        if (finishReason === 'SAFETY') {
          message =
            'The response was blocked due to safety settings. Please adjust your prompt and try again.'
  ...
  const handleAIFeatureClick = (type) => {
    ...
    if (type === 'recipe') {
      ...
      promptGenerator = (input) =>
        `Generate a creative recipe based on the following ingredients: ${input}. Provide a catchy name for the recipe, a short description, a list of ingredients, and step-by-step instructions.`
    } else if (type === 'learning') {
      title = '✨ AI Learning Planner'
      promptGenerator = (input) =>
        `I want to learn "${input}". Break this down into a structured, step-by-step learning plan. Include key topics, suggested resources (like books, online courses, or projects), and a logical progression from beginner to advanced.`
    } else if (type === 'schedule') {
      title = '✨ AI Day Planner'
      promptGenerator = (input) =>
        `Generate a time-blocked daily schedule for me. My main tasks for today are: ${input}. Be sure to include breaks, and organize the tasks logically (e.g., deep work in the morning). Present it in a clear, easy-to-read format.`
    }
  ...
  const alternatingFeatures = [
    ...
    },
    {
      ...
      points: [
        'Track progress by pages or modules',
    ...
    <div className="min-h-screen w-full bg-slate-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 flex flex-col items-center font-sans">
      ...
      <section className="w-full max-w-5xl mx-auto px-4 pt-40 pb-16 flex flex-col items-center text-center animate-fade-in">
        ...
        <p className="text-lg md:text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">
          The ultimate productivity platform to help you build routines, achieve
          your goals, and become the best version of yourself.
        </p>
      ...
      <section className="w-full max-w-6xl mx-auto px-4 pt-32 pb-16 flex flex-col md:flex-row items-center gap-12 md:gap-24">
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center gap-2 text-purple-600 font-medium">
                <input
                  type="checkbox"
                  readOnly
                  className="accent-purple-500 w-5 h-5 rounded"
                />
                Meditate
              </label>
                ...
                {[...Array(5)].map((_, row) => (
                  <tr key={row}>
                    {[...Array(7)].map((_, col) => {
                      ...
                      const isHabitDay = [
                        2, 3, 8, 9, 10, 15, 17, 18, 22, 23, 24, 29, 30,
                      ].includes(day)
                      const isToday = day === 10
                      return (
                        <td
                          key={col}
                          className={`py-1 px-2 rounded-lg ${isToday ? 'bg-blue-100 dark:bg-blue-900 font-bold' : ''} ${isHabitDay ? 'text-green-600 font-bold' : ''}`}
                        >
                          {row === 0 && col === 0 ? '' : day > 31 ? '' : day}
                        </td>
                      )
                    })}
                  </tr>
        ...
        <div className="flex-1 flex flex-col items-start justify-center max-w-md w-full mt-12 md:mt-0">
          ...
          <p className="text-lg text-gray-700 dark:text-gray-300 mb-4">
            Build habits, track your goals, and organize your life with Momentum
            Grid.
            <br />
            Visualize your progress, stay motivated, and craft your ideal
            day—habit by habit.
          </p>
      ...
      {modalState.isOpen && (
        <div
          className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex justify-center items-center z-50 animate-fade-in-fast"
          onClick={closeModal}
        >
          <div
            className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-6 md:p-8 max-w-2xl w-full mx-4 transform animate-fade-in-up"
            ...
            {isLoading ? (
              ...
              <div>
                <textarea
                  ...
                  placeholder={
                    modalState.type === 'recipe'
                      ? 'e.g., chicken breast, rice, broccoli, soy sauce'
                      : modalState.type === 'learning'
                        ? 'e.g., Learn Python for Data Science'
                ...
                <button
                  ...
                  disabled={!userInput}
                  className="mt-4 w-full px-6 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
                  Generate
                </button>
              </div>
