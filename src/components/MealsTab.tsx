...
const MealsTab = () => {
  ...
  const saveMealEntry = async () => {
    ...
    try {
      if (existingEntry?.id != null) {
        ...
        setMealEntries((prev) =>
          prev.map((e) =>
            e.id === existingEntry.id ? ((data as MealEntry) ?? e) : e,
          ),
        )
      } else {
        // Insert new entry
  ...
  const getDayStatus = (date: Date) => {
    ...
    const loggedMeals = NON_SNACK_MEAL_TYPES.filter(
      (type) => entry.meals[type].logged,
    ).length
            ...
            <Card className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border dark:border-gray-700">
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center space-x-2">
                  ...
                  <span className="font-semibold text-base lg:text-lg dark:text-gray-100">
                    Meal Calendar
                  </span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-xs lg:text-sm text-gray-600 dark:text-gray-300 mb-4">
                  ...
                  <span className="font-bold text-green-500">Green</span> for
                  complete days,{' '}
                  <span className="font-bold text-yellow-500">Yellow</span> for
                  partial days.
                </p>
                <CalendarView
                    ...
                    {mealForm[meal.key].logged && (
                      <div className="space-y-2">
                        <Input
                          ...
                          onChange={(e) =>
                            updateMealDescription(meal.key, e.target.value)
                          }
                          ...
                          <div className="flex flex-wrap gap-1">
                            {mealCategories.map((category) => (
                              <button
                                key={category.value}
                                onClick={() =>
                                  updateMealCategory(meal.key, category.value)
                                }
            ...
            <div className="flex-shrink-0 border-t dark:border-gray-700 pt-4">
              <div className="flex justify-between items-center space-x-2">
                ...
                <div className="flex space-x-2">
                  ...
                  <Button onClick={saveMealEntry} className="text-sm">
                    Save Meals
                  </Button>
                </div>
              </div>
