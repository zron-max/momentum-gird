...
const RoutinesTab = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const routinesWithTasks = (routinesData || []).map((r) => ({
          ...r,
          tasks: r.tasks || [],
        }))
        const entriesWithTasks = (entriesData || []).map((e) => ({
          ...e,
  ...
  const calculateStreaks = (routineId: string) => {
    ...
    const completedDates = routineEntries
      .filter(
        (e) =>
          e.routine_id === routineId &&
          e.completed_tasks.length === routine.tasks.length,
      )
      .map((e) => e.date)
  ...
  const addRoutine = async () => {
    if (
      ...
      !newRoutine.tasks.some((t) => t.trim())
    )
      return
  ...
  const updateRoutine = async () => {
    if (
      !user ||
      !routineToEdit ||
      !routineToEdit.title.trim() ||
      !routineToEdit.tasks.some((t) => t.trim())
    )
      return
  ...
  const deleteRoutine = async () => {
    ...
    try {
      ...
      setRoutineEntries(
        routineEntries.filter((e) => e.routine_id !== routineId),
      )
  ...
  const toggleTask = async (task: string) => {
    ...
    const entry = routineEntries.find(
      (e) => e.date === dateString && e.routine_id === selectedRoutine,
    )

    const updatedTasks = entry
  ...
  const getDayStatus = (date: Date) => {
    ...
    const completionRate =
      entry.completed_tasks.length / selectedRoutineData.tasks.length
  ...
  const isTodayComplete =
    ...
    routineEntries.find(
      (e) =>
        e.routine_id === selectedRoutine &&
        e.date === new Date().toISOString().split('T')[0] &&
        e.completed_tasks.length === selectedRoutineData.tasks.length,
    )
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Track your daily routines and build consistent habits
          </p>
        </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-2">
                          <div
                            className={`w-3 h-3 rounded-full ${routine.color}`}
                          />
                        ...
                        <div className="flex items-center space-x-3 text-xs text-gray-600 dark:text-gray-300">
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            <span>
                              {calculateStreaks(routine.id).currentStreak} day
                              streak
                            </span>
                          </div>
                        </div>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <p>
                ...
                <strong>{routineToDelete?.title}</strong>"? This will
                permanently remove the routine and all of its associated
                entries. This action cannot be undone.
              </p>
              <div className="flex justify-end space-x-2">
                <Button
                  variant="outline"
                  onClick={() => setIsDeletingRoutine(false)}
                >
                  Cancel
                </Button>
          ...
          {selectedRoutineData ? (
            <div className="space-y-4">
              <div className="bg-white dark:bg-gray-900 rounded-lg p-4 shadow-sm border dark:border-gray-700">
                ...
                <div className="flex items-center space-x-3 text-sm text-gray-600 dark:text-gray-200">
                  <span>{selectedRoutineData.tasks.length} tasks</span>
                  <div className="flex items-center space-x-1">
                    <Trophy className="w-3 h-3 text-yellow-500" />
                    <span>
                      {calculateStreaks(selectedRoutineData.id).currentStreak}{' '}
                      day streak
                    </span>
                  </div>
                </div>
                  ...
                  {selectedDate && (
                    <div className="space-y-2">
                      {selectedRoutineData.tasks.map((task, index) => {
                        const isCompleted =
                          selectedDateEntry?.completed_tasks.includes(task) ||
                          false
                        return (
                          <div
                            key={index}
                          ...
                          >
                            ...
                            <span
                              className={`text-sm ${isCompleted ? 'line-through text-gray-500 dark:text-gray-400' : 'dark:text-gray-100'}`}
                            >
                              {task}
                            </span>
                          </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg p-8 text-center border dark:border-gray-700">
              <p className="text-muted-foreground dark:text-gray-400">
                Select a routine to start tracking
              </p>
            </div>
          )}
        </div>
