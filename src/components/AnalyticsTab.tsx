...
const AnalyticsTab = () => {
    ...
    const [weeklyStats, setWeeklyStats] =
        useState<WeeklyStats>(defaultWeeklyStats);
    ...
    async function safeSelect(table: string, cols = "*", filters: any[] = []) {
        try {
            ...
            if (error) {
                ...
                console.warn(
                    `safeSelect: error selecting from ${table}:`,
                    error.message || error
                );
    ...
    useEffect(() => {
        const fetchAll = async () => {
            ...
            try {
                ...
                const habitsRes = await safeSelect(
                    "habits",
                ...
                if (!habitsRes.error && Array.isArray(habitsRes.data)) {
                    ...
                    habitsCompleted = arr.filter(
                        (r) => r.completed === true
                    ).length;
                } else {
                    // fallback: try "habit_logs" or nothing
                    console.info(
                        "habits table not found or empty. using 0 fallback."
                    );
                }

                // Learning: try "learning_sessions" or "learning"
                const learningResA = await safeSelect(
                    "learning_sessions",
                    "id,user_id,progress_percent",
                    [["eq", "user_id", user.id]]
                );
                const learningResB = await safeSelect(
                    "learning",
                    "id,user_id,progress_percent",
                    [["eq", "user_id", user.id]]
                );
                ...
                if (
                    !learningResA.error &&
                    Array.isArray(learningResA.data) &&
                    learningResA.data.length > 0
                ) {
                    // average progress_percent if present
                    const arr = learningResA.data as any[];
                    const valid = arr.filter(
                        (r) => typeof r.progress_percent === "number"
                    );
                    if (valid.length > 0) {
                        learningCompletion = Math.round(
                            valid.reduce((s, x) => s + x.progress_percent, 0) /
                                valid.length
                        );
                    } else {
                        learningCompletion = Math.min(
                            100,
                            Math.round((arr.length / 10) * 100)
                        ); // heuristic
                    }
                } else if (
                    !learningResB.error &&
                    Array.isArray(learningResB.data) &&
                    learningResB.data.length > 0
                ) {
                    ...
                    console.info(
                        "learning tables not found or empty - fallback to 0"
                    );
                }

                // Projects: compute percent complete from project_milestones (works with your existing schema)
                const projectsRes = await safeSelect("projects", "id,user_id", [
                    ["eq", "user_id", user.id],
                ]);
                ...
                if (
                    ...
                    if (projectsArr.length === 0) {
                        ...
                        if (perProject.length) {
                            projectsPercent = Math.round(
                                perProject.reduce((s, x) => s + x, 0) /
                                    perProject.length
                            );
                        } else {
                            projectsPercent = 0;
                    ...
                    console.info(
                        "projects or project_milestones missing, fallback to 0"
                    );
                }

                // Routines: try "routines" with completed boolean
                const routinesResB = await safeSelect(
                    "routines",
                ...
                if (
                    !routinesResB.error &&
                    Array.isArray(routinesResB.data) &&
                    routinesResB.data.length > 0
                ) {
                    ...
                    routinesPercent = total
                        ? Math.round((completed / total) * 100)
                        : 0;
                } else {
                    console.info("routines table missing, fallback to 0");
                }
                ...
                if (!mealsRes.error && Array.isArray(mealsRes.data)) {
                    ...
                    if (last7.length > 0) {
                        ...
                        mealsPercent = Math.round(
                            (completeDays / last7.length) * 100
                        );
                    } else {
                        mealsPercent = 0;
                    }
                    ...
                    for (const e of arr) {
                        try {
                            ...
                            for (const mKey of Object.keys(meals)) {
                                ...
                                if (cat)
                                    mealCategoryCounts[cat] =
                                        (mealCategoryCounts[cat] || 0) + 1;
                            }
                        } catch (err) {
                            // skip
                        }
                    ...
                    console.info(
                        "meal_entries table missing or empty - fallback to 0"
                    );
                }
                ...
                const trends = [
                    {
                        ...
                        completion: Math.round(
                            habitsTotal
                                ? (habitsCompleted / Math.max(1, habitsTotal)) *
                                      100
                                : 0
                        ),
                        change: "+0%",
                    ...
                    {
                        category: "Learning",
                        completion: learningCompletion || 0,
                        change: "+0%",
                    },
                    {
                        category: "Projects",
                        completion: projectsPercent || 0,
                        change: "+0%",
                    },
                    {
                        category: "Routines",
                        completion: routinesPercent || 0,
                        change: "+0%",
                    },
                    {
                        category: "Meals",
                        completion: mealsPercent || 0,
                        change: "+0%",
                    },
                ];
                ...
                if (habitsCompleted >= 7)
                    achievements.push({
                        title: `${habitsCompleted}-day habit streak`,
                        subtitle: "Nice consistency!",
                        color: COLORS.habits,
                        emoji: "🔥",
                    });
                if (learningCompletion >= 50)
                    achievements.push({
                        title: `${learningCompletion}% learning goal`,
                        subtitle: "Keep going!",
                        color: COLORS.learning,
                        emoji: "📚",
                    });
                if (projectsPercent >= 50)
                    achievements.push({
                        title: "Project momentum",
                        subtitle: "Milestones being completed",
                        color: COLORS.projects,
                        emoji: "🎯",
                    });
                if (routinesPercent >= 80)
                    achievements.push({
                        title: "Routines rock",
                        subtitle: `${routinesPercent}% routine adherence`,
                ...
                if (mealsPercent >= 70)
                    achievements.push({
                        title: "Healthy days",
                        subtitle: `${mealsPercent}% days complete`,
                        color: COLORS.meals,
                        emoji: "🥗",
                    });

                // Pie data from mealCategoryCounts
                const pie = Object.entries(mealCategoryCounts).map(
                    ([k, v]) => ({
                        name: k,
                        value: v,
                        color:
                ...
                setWeeklyStats({
                    habits: Math.round(
                        habitsTotal
                            ? (habitsCompleted / Math.max(1, habitsTotal)) * 100
                            : 0
                    ),
                        ...
                        : [
                              {
                                  ...
                                  subtitle: "Start logging to see achievements",
                ...
                setPieData(
                    pie.length
                        ? pie
                        : [{ name: "No data", value: 1, color: COLORS.neutral }]
                );
            } catch (err) {
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-muted-foreground mt-1">
                    Track your progress and identify trends across all areas
                </p>
            </div>
                ...
                {[
                    {
                        key: "habits",
                        label: "Daily Tasks",
                        value: weeklyStats.habits,
                        color: COLORS.habits,
                        icon: <BarChart3 className="w-4 h-4 text-current" />,
                    },
                    {
                        key: "learning",
                        label: "Learning",
                        value: weeklyStats.learning,
                        color: COLORS.learning,
                    ...
                    },
                    {
                        key: "projects",
                        label: "Projects",
                        value: weeklyStats.projects,
                        color: COLORS.projects,
                        icon: <Target className="w-4 h-4 text-current" />,
                    },
                    {
                        key: "routines",
                        label: "Routines",
                        value: weeklyStats.routines,
                        color: COLORS.routines,
                        icon: <Calendar className="w-4 h-4 text-current" />,
                    },
                    {
                        key: "meals",
                        label: "Meals",
                        ...
                        <CardContent>
                            <div className="flex items-center justify-between">
                                ...
                                <div
                                    ...
                                    style={{
                                        backgroundColor: `${color}20` /* slight tint */,
                                    }}
                                >
                                    {icon}
                                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Completion rates and changes from last month
                        </p>
                    </CardHeader>
                    <CardContent>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-lg">
                            Breakdowns & Recent Achievements
                        </CardTitle>
                        <p className="text-sm text-muted-foreground">
                            Meal categories and your latest wins
                        </p>
                    </CardHeader>
                    <CardContent>
            ...
            <Card>
                ...
                <CardContent>
                    <div className="text-center py-8 text-muted-foreground">
                        ...
                        <p className="text-sm">
                            Advanced charts and detailed analytics coming soon!
                        </p>
                        <p className="text-xs mt-1">
                            Including heatmaps, trend lines, and custom date
                            ranges.
                        </p>
                    </div>
                </CardContent>
