...
const AnalyticsTab = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        let mealsPercent = 0
        const mealCategoryCounts: Record<string, number> = {}
        let mealsArr: any[] = []
        if (!mealsRes.error && Array.isArray(mealsRes.data)) {
          ...
          if (last7.length > 0) {
            ...
            mealsPercent = Math.round(
              (completeDays / Math.max(1, last7.length)) * 100,
            )
          } else {
            mealsPercent = 0
          }
          ...
          for (const e of mealsArr) {
            try {
              ...
              for (const mKey of Object.keys(meals)) {
                ...
                if (cat)
                  mealCategoryCounts[cat] = (mealCategoryCounts[cat] || 0) + 1
              }
            } catch {}
          }
        } else {
        ...
        if (
          ...
          if (projectsArr.length === 0) {
            ...
            projectsPercent = perProject.length
              ? Math.round(
                  perProject.reduce((s, x) => s + x, 0) / perProject.length,
                )
              : 0
          }
        } else {
        ...
        const habitsRes = await safeSelect('habits', 'id,user_id', [
          ['eq', 'user_id', user.id],
        ])
        ...
        if (
          ...
          const possibleCount = distinctHabits.length * daysWindow
          if (possibleCount === 0) {
            habitsPercent = 0
          } else {
            ...
            habitsPercent = Math.round(
              (completedCount / Math.max(1, possibleCount)) * 100,
            )
          }
        } else {
        ...
        const learningGoalsRes = await safeSelect(
          'learning_goals',
          'id,user_id,target_value',
          [['eq', 'user_id', user.id]],
        )
        const learningEntriesRes = await safeSelect(
          'learning_entries',
          'id,goal_id,user_id,value',
          [['eq', 'user_id', user.id]],
        )
        ...
        if (
          ...
          if (goals.length === 0) {
            ...
            learningCompletion = Math.round(
              perGoal.reduce((s, x) => s + x, 0) / perGoal.length,
            )
          }
        } else {
          console.info(
            'learning_goals or learning_entries missing - fallback to 0',
          )
        }
        ...
        const routinesRes = await safeSelect('routines', 'id,user_id,tasks', [
          ['eq', 'user_id', user.id],
        ])
        ...
        if (
          ...
          if (routineIds.length === 0) {
            ...
            const daysCounts: number[] = [] // fraction per day
            for (let i = 0; i < daysWindow; i++) {
              ...
              for (const r of routinesArr) {
                ...
                const re = routineEntriesArr.find(
                  (e: any) =>
                    String(e.routine_id) === String(r.id) &&
                    String(e.date) === dateStr,
                )
                if (
                  re &&
                  Array.isArray(re.completed_tasks) &&
                  re.completed_tasks.length >= rTasks.length &&
                  rTasks.length > 0
                ) {
                  completedRoutines++
                }
              ...
              daysCounts.push(
                (completedRoutines / Math.max(1, routineIds.length)) * 100,
              )
            }
            // average across days
            routinesPercent = Math.round(
              daysCounts.reduce((s, x) => s + x, 0) /
                Math.max(1, daysCounts.length),
            )
          }
        } else {
        ...
        const trends = [
          { category: 'Habit', completion: habitsPercent, change: '+0%' },
          {
            category: 'Learning',
        ...
        if (habitsPercent >= 70)
          achievements.push({
            title: `Consistent: ${habitsPercent}% habits`,
            subtitle: 'Good job!',
            color: COLORS.habits,
            emoji: '🔥',
          })
        ...
        if (mealsPercent >= 70)
          achievements.push({
            title: 'Healthy days',
            subtitle: `${mealsPercent}% complete`,
            color: COLORS.meals,
            emoji: '🥗',
          })
  ...
  const barChartData = useMemo(
    () =>
      monthlyTrends.map((t) => ({
        name: t.category,
        completion: toNumber(t.completion, 0),
      })),
    [monthlyTrends],
  )
                    ...
                    >
                      {pieChartData.map((entry, index) => (
                        <Cell
                          ...
                          fill={
                            entry.color ||
                            (index % 2 ? COLORS.learning : COLORS.habits)
                          }
                        />
                      ))}
                    </Pie>
                ...
                {achievementList.map((a, i) => (
                  ...
                  >
                    <div
                      ...
                      style={{ background: a.color || COLORS.neutral }}
                    >
        ...
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-xs mt-1">
              Including heatmaps, trend lines, and custom date ranges.
            </p>
          </div>
        </CardContent>
      </Card>
